return {
  ...
  { 'folke/trouble.nvim', cmd = 'Trouble', opts = {}, keys = { { '<leader>xx', '<cmd>Trouble diagnostics toggle<cr>', desc = 'diagnostics toggle' } } },
  {
    'mikavilpas/yazi.nvim',
    event = 'VeryLazy',
    keys = { { '<leader>-', '<cmd>Yazi<cr>', desc = 'open yazi' }, { '<leader>_', '<cmd>Yazi cwd<cr>', desc = 'open yazi (cwd)' } },
    opts = { opener = function(url) require('yazi').open(url) end },
  },
  { 'vim-test/vim-test', keys = { { '<leader>tn', '<cmd>TestNearest<cr>' }, { '<leader>tf', '<cmd>TestFile<cr>' } } },
  { 'preservim/vimux', cmd = { 'VimuxRunCommand', 'VimuxPromptCommand' }, keys = { { '<leader>vp', '<cmd>VimuxPromptCommand<cr>' } } },
  { 'rafamadriz/friendly-snippets' },
  {
    ...
    config = function()
      ...
      local function apply_ash_adjust()
        ...
        local fg_main, fg_dim, fg_dark, bg_status, bg_none, black, bg_hl = '#C6C6C6', '#707070', '#3A3A3A', '#151515', 'none', '#030303', '#1A1A1A'
    ...
    config = function()
      ...
      require('nvim-autopairs').add_rules {
        Rule(' ', ' ')
          ...
          :with_move(function(opts) return opts.prev_char == ' ' end)
  ...
  },
  {
    ...
    opts = { colorcolumn = '100', disabled_filetypes = { 'help', 'text', 'markdown', 'lazy', 'mason', 'neo-tree', 'TelescopePrompt' }, scope = 'window' },
  },
  {
  ...
  },
  {
    ...
    init = function() vim.g.tmc_cli_path = '/usr/local/bin/tmc-langs-cli' end,
    keys = { { '<leader>tt', ':TmcRunTests<CR>', desc = 'tmc: run tests' }, { '<leader>ts', ':TmcSubmit<CR>', desc = 'tmc: submit solution' } },
  },
  ...
  { 'windwp/nvim-ts-autotag', opts = {} },
  {
    ...
    config = function()
      require('aerial').setup {
        ...
        filter_kind = { 'Class', 'Constructor', 'Enum', 'Field', 'Function', 'Interface', 'Method', 'Module', 'Namespace', 'Struct' },
  ...
  },
  {
    ...
    config = function()
      ...
      local function desaturate_hex(hex)
        if not hex or hex == '' then return '#888888' end
        hex = hex:gsub('#', '')
        local r, g, b = tonumber(hex:sub(1, 2), 16) or 0, tonumber(hex:sub(3, 4), 16) or 0, tonumber(hex:sub(5, 6), 16) or 0
      ...
      if icons_by_extension then
        for key, data in pairs(icons_by_extension) do
          custom_overrides_ext[key] = { icon = data.icon, color = desaturate_hex(data.color), name = data.name }
        end
      end

      devicons.setup { color_icons = true, override = custom_overrides, override_by_extension = custom_overrides_ext }
    end,
  },
  {
                ...
                kind_icon = {
                  text = function(ctx)
                    ...
                    if ctx.item.source_name == 'LSP' and ctx.item.documentation then
                      ...
                      if color_item and color_item.abbr and color_item.abbr ~= '' then icon = color_item.abbr end
                    end
  ...
  { 'folke/todo-comments.nvim', event = 'VimEnter', dependencies = { 'nvim-lua/plenary.nvim' }, opts = { signs = false } },
  {
    ...
    config = function()
      ...
      require('mini.files').setup { windows = { width_focus = 30, width_nofocus = 15 }, options = { use_as_default_explorer = true } }
      vim.keymap.set('n', '<leader>e', function() require('mini.files').open(vim.api.nvim_buf_get_name(0)) end, { desc = 'Open mini.files (current file)' })
      vim.keymap.set('n', '<leader>E', function() require('mini.files').open() end, { desc = 'Open mini.files (cwd)' })
      ...
      statusline.setup {
        ...
        content = {
          active = function()
            ...
            if git_dict then
              local diffs = {}
              if git_dict.added and git_dict.added > 0 then table.insert(diffs, '+' .. git_dict.added) end
              if git_dict.changed and git_dict.changed > 0 then table.insert(diffs, '~' .. git_dict.changed) end
              if git_dict.removed and git_dict.removed > 0 then table.insert(diffs, '-' .. git_dict.removed) end
              if #diffs > 0 then git = git .. '  ' .. table.concat(diffs, ' ') end
            end
            local diagnostics =
              MiniStatusline.section_diagnostics { trunc_width = 75, icon = '', signs = { ERROR = '󰅚 ', WARN = '󰀪 ', INFO = '󰋽 ', HINT = '󰌶 ' } }
            ...
            if vim.g.have_nerd_font and fileinfo ~= '' then
              ...
              if icon then fileinfo = icon .. ' ' .. fileinfo end
            end
